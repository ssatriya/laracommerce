...
const EditProductImage = ({ files, setFiles, error, productId }: Props) => {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        router.put(
            route('dashboard.images.reorder.existing', productId),
            { items: orderedIds },
            {
                preserveScroll: true,
                preserveState: true,
    ...
    const handleUploadImage = async (file: FileWithPreview) => {
        ...
        formData.append('order', String(files.length + 1));
        formData.append('id', file.id);

        router.post(route('dashboard.images.upload.existing', productId), formData, {
            forceFormData: true,
            preserveScroll: true,
            preserveState: true,
            onError: (errors) => {
